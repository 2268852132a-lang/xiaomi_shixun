...
volatile uint16_t  Uart0_RecvLenth = 0;
volatile UartRecvState_t Uart0_RecvFlag;
volatile uint16_t  Uart0_RecvCnt = 0;

uint8_t   Uart5_RecvBuffer[512];
volatile uint16_t  Uart5_RecvLenth = 0;
volatile UartRecvState_t Uart5_RecvFlag;
volatile uint16_t  Uart5_RecvCnt = 0;
...
*函数名:    uart_init
*功能:      初始化相应串口
*入口参数:  usart_periph   USART0/USART5
*出口参数： 无
*返回值：   无
*作者：     ZZZ
*日期:      2023/4/6
...
{
		volatile UartRecvState_t *recvflag;
		uint8_t *recvbuf;
		volatile uint16_t *recvlen;
		uint16_t ret_len = 0;
...
*功能:      串口5中断函数,接收串口数据
*入口参数:  中断相应数据
*出口参数： Uart5_RecvBuffer存放接收数据,Uart5_RecvLenth接收数据长度
*返回值：   无
*作者：     ZZZ
*日期:      2023/4/6
**********************************************************************************************/
